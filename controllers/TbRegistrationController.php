...
use Yii;
use yii\base\Model;
use app\models\TbInfo;
use app\models\TbInfoSearch;
use app\models\TbClinical;
...
/**
 * RegistryController implements the CRUD actions for TbInfo model.
 */
    ...
    public function actionCreate()
    {
           ...
           Model::validateMultiple([$modelInfo,$modelClinical]))
        {
            ...
            if($modelInfo->validate()){
                ...
                $modelClinical->save();
                $tbno=$modelClinical->TBNUMBER;
                $date_start=$modelClinical->MDTDATE;
                $hno=$modelInfo->HNO;
                $village_id=$modelInfo->VILLA;
                $villagename = $modelInfo->getVillageName();
                $subdistrictname = $modelInfo->getSubdistrictName();
                $pcu = $modelInfo->getHospcode();
                $message= "แจ้งขึ้นทะเบียนผู้ป่วยใหม่   เลขทะเบียนวัณโรค   ".$tbno." วันที่เริ่มกินยา ".$date_start."  บ้านเลขที่   ".$hno."  หมู่ที่  ".$village_id."  บ้าน  ".$villagename." ตำบล  ".$subdistrictname."   รหัสหน่วยบริการที่รับผิดชอบ ".$pcu;
